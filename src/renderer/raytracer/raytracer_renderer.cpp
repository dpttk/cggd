...
void cg::renderer::ray_tracing_renderer::render()
{
	...
	raytracer->miss_shader = [](const ray& ray) {
		...
		payload.color = {0.f, 0.f, 0.f}; // CREATIVE
		return payload;
	};


	std::random_device random_device; // CREATIVE
	std::mt19937 random_generator(random_device()); // CREATIVE
	std::uniform_real_distribution<float> uniform_distribution(-1.f, +1.f); // CREATIVE
	...
	raytracer->closest_hit_shader = [&](const ray& ray, payload& payload, const triangle<cg::vertex>& triangle, size_t depth) {
		...
		float3 random_direction{
				...
				uniform_distribution(random_generator),
				uniform_distribution(random_generator)}; // CREATIVE
		if (dot(normal, random_direction) < 0.f)
			random_direction = -random_direction; // CREATIVE

		cg::renderer::ray to_next_object(position, random_direction); // CREATIVE 
		auto next_payload = raytracer->trace_ray(to_next_object, depth); // CREATIVE 
		result_color += triangle.diffuse * next_payload.color.to_float3() *
						std::max(dot(normal, to_next_object.direction), 0.f); // CREATIVE 
